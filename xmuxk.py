        ...
        if not watch_list:
            ...
            for course_info in watch_list[:]:  # 做副本，避免迭代时删除元素
                ...
                if not jxbid or not teaching_class_type:
                    console.print(
                        f"[red]监听列表中缺少 JXBID 或 clazzType，跳过此条课程。[/red]"
                    )
                    continue
                ...
                if found_course:
                    ...
                    console.print(
                        f"[bold]已选人数[/bold]: {number_of_selected} / [bold]容量[/bold]: {class_capacity}"
                    )
                    ...
                    console.print(
                        f"[red]未找到课程 {course_name} (JXBID={jxbid})，可能已下架或查询条件有误。[/red]"
                    )
                    return
...
def main():
    ...
    parser.add_argument(
        "--watch", action="store_true", help="监听模式（交互式添加监听课程）"
    )
    parser.add_argument(
        "--listen", action="store_true", help="是否进入持续监听循环（自动查询）"
    )
    parser.add_argument(
        "--interval", type=int, default=60, help="监听模式下的查询间隔秒数，默认 60 秒"
    )
    parser.add_argument("--type", help="选课类型的显示名称，例如：本专业计划课程")
    parser.add_argument("--key", help="搜索关键词")
    parser.add_argument(
        "--autoadd", action="store_true", help="是否启用自动添加监听课程"
    )
    parser.add_argument(
        "--random", action="store_true", help="是否启用随机调整查询间隔"
    )
    ...
    while True:
        ...
        course_controller = XMUCourseController(
            xmu.session, xmu.token, xmu.batch_id, xmu.cookies
        )
        ...
        if args.listen:
            listen_loop(
                ...
                course_controller,
                args.interval,
                autoadd_enabled=args.autoadd,
                random_adjustment=args.random,
            )
